...
exports.handler = async (event)=>{
  ...
  try{
    ...
    if(!body.item_id) return bad(400,"Missing item_id");
    const tok = await rest(`whq_plaid_tokens?item_id=eq.${encodeURIComponent(body.item_id)}&household_id=eq.${hh}&select=access_token`);
    if(tok.length){ try{ await plaid("/item/remove",{ access_token:tok[0].access_token }); }catch(e){} }
    await rest(`whq_plaid_tokens?item_id=eq.${encodeURIComponent(body.item_id)}&household_id=eq.${hh}`,"DELETE");
    await rest(`whq_plaid_items?item_id=eq.${encodeURIComponent(body.item_id)}&household_id=eq.${hh}`,"DELETE");
