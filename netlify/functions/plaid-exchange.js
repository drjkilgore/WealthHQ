...
exports.handler = async (event)=>{
  ...
  try{
    ...
    if(!body.public_token) return bad(400,"Missing public_token");
    const j = await plaid("/item/public_token/exchange",{ public_token:body.public_token });
    await rest("whq_plaid_tokens","POST",{ item_id:j.item_id, household_id:hh, access_token:j.access_token },"resolution=merge-duplicates");
    await rest("whq_plaid_items","POST",{ item_id:j.item_id, household_id:hh, institution_name:body.institution_name||"Bank" },"resolution=merge-duplicates");
    return ok({ item_id:j.item_id });
